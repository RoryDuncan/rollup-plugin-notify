...
if (VERSION) {
  let [major, minor, _patch] = VERSION.split(".").map(Number);
  if (major < 1 && minor < 60)
    console.warn(
      `'rollup-plugin-notify' will not work. Rollup 0.60.0 or higher is required`
    );
}
...
function calculateCountOfReplacementChar(string, replaceByChar = " ") {
  ...
  if (replaceByChar !== " ")
    characters = characters / (charSizes.get(replaceByChar) || 1);
...
function extractMessage(error) {
  ...
  if (error.plugin === "babel") {
    ...
    if (message.includes(filepath)) {
      ...
      if (message.includes(filepath)) stripFilePath();
    }
  }
  return message;
}
...
function getFileName(filepath) {
  if (typeof filepath === "string") return path.parse(filepath).base;
}
...
function createCodeBlock(frame) {
  ...
  if (caretCountOriginal === 1) {
    ...
    var caretCountNew = calculateCountOfReplacementChar(
      toBeReplacedByMarkers,
      "^"
    );
...
function notifyError(error, buildId) {
  ...
  if (filepath) titleSections.push(getFileName(filepath));
  if (line !== undefined && column !== undefined)
    titleSections.push(`(${line}:${column})`);
  if (error.plugin) titleSections.push(error.plugin);
  else if (error.code) titleSections.push(error.code);
